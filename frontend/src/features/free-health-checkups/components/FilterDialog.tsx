...
import type { BloodDonationFilters } from "../types";

type FilterDialogProps = {
  open: boolean;
  filters: BloodDonationFilters;
  onOpenChange: (open: boolean) => void;
  onApply: (filters: BloodDonationFilters) => void;
  onReset: () => void;
};
...
const optionsMap: Record<keyof BloodDonationFilters, string[]> = {
  city: ["Mumbai", "Chennai", "Pune"],
  donationType: ["walk-in", "appointment"],
};

export function FilterDialog({ open, filters, onOpenChange, onApply, onReset }: FilterDialogProps) {
  const [selectedCategory, setSelectedCategory] = useState<typeof categories[number]>(categories[0]);
  const [localFilters, setLocalFilters] = useState<BloodDonationFilters>(filters);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[92vw] lg:max-w-4xl">
        <DialogHeader className="gap-3 pb-4">
          ...
          <DialogDescription>Refine the camp list by city and donation type.</DialogDescription>
        </DialogHeader>
