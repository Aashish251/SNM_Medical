...
type CampCardProps = {
  ...
  onViewDetails: (camp: BloodDonationCamp) => void;
  onRegister: (camp: BloodDonationCamp) => void;
};

export function CampCard({ camp, onViewDetails, onRegister }: CampCardProps) {
  ...
  return (
    <Card className="flex h-full flex-col overflow-hidden rounded-2xl border-border/80 shadow-md transition-shadow hover:shadow-lg">
      ...
      <CardFooter className="grid gap-3 p-5 pt-0">
        ...
        <Button className="h-11 w-full rounded-xl font-semibold bg-to-two-right-theme-gradient to-yellow-200 shadow-md text-white font-bold text-base transition-transform duration-300 " onClick={() => onRegister(camp)}>
          {actionLabel}
        </Button>
      </CardFooter>
    </Card>
