...
type CampRegistrationDialogProps = {
  open: boolean;
  camp: Camp | null;
...
export function CampRegistrationDialog({
  open,
  camp,
  labels,
  onOpenChange,
}: CampRegistrationDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle>{camp ? actionLabel : labels.registrationDialogTitle}</DialogTitle>
          <div className="text-sm leading-relaxed text-muted-foreground">
            {camp
              ? labels.registrationDialogDescription(camp.title)
              : "Select a camp to continue your registration."}
          </div>
        </DialogHeader>
